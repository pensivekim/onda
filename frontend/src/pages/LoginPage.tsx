      ...
      <div className="flex items-center justify-center px-6 py-20">
        <div className="bg-surface-card rounded-card p-10 w-full max-w-sm text-center" style={{ boxShadow: '0px 12px 32px rgba(25,28,29,0.06)' }}>
          ...
          {(() => {
            ...
            const kakaoBtn = (
              <a key="kakao" href={`${api.API_URL}/api/auth/kakao`}
            ...
            const googleBtn = (
              <a key="google" href={`${api.API_URL}/api/auth/google`}
            ...
            const lineBtn = (
              <a key="line" href={`${api.API_URL}/api/auth/line`}
          ...
          <p className="text-xs text-on-surface-variant mt-8 leading-relaxed">{t('termsAgree')}</p>
        </div>
      </div>
    </div>
  );
